...
class PPO(Agent):
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._rnn_sequence_length = (
            self.policy.get_specification().get("rnn", {}).get("sequence_length", 1)
        )

        # policy
        for i, size in enumerate(
            self.policy.get_specification().get("rnn", {}).get("sizes", [])
        ):
            self._rnn = True
            # create tensors in memory
            if self.memory is not None:
                self.memory.create_tensor(
                    name=f"rnn_policy_{i}",
                    size=(size[0], size[2]),
                    dtype=torch.float32,
        ...
        if self.value is not None:
            if self.policy is self.value:
                ...
                for i, size in enumerate(
                    self.value.get_specification().get("rnn", {}).get("sizes", [])
                ):
                    self._rnn = True
                    # create tensors in memory
                    if self.memory is not None:
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for epoch in range(self._learning_epochs):
            ...
            for i, (
                    ...
                    if self.policy is self.value:
                        ...
                        rnn_policy = {
                            "rnn": [
                                s.transpose(0, 1)
                                for s, n in zip(
                                    sampled_rnn_batches[i], self._rnn_tensors_names
                                )
                                if "policy" in n
                            ],
                            "terminated": sampled_dones,
                ...
                # compute policy loss
                ratio = torch.exp(next_log_prob - sampled_log_prob[indices])
                # surrogate = sampled_advantages * ratio
                # surrogate_clipped = sampled_advantages * torch.clip(
                #     ratio, 1.0 - self._ratio_clip, 1.0 + self._ratio_clip
                # )
                surrogate = sampled_advantages[indices] * ratio
                surrogate_clipped = sampled_advantages[indices] * torch.clip(
                    ratio, 1.0 - self._ratio_clip, 1.0 + self._ratio_clip
                )
                ...
                if self._clip_predicted_values:
                    ...
                    # )
                    predicted_values = sampled_values[indices] + torch.clip(
                        predicted_values - sampled_values[indices],
                        min=-self._value_clip,
                        max=self._value_clip,
                    )
                ...
                # )
                value_loss = self._value_loss_scale * F.mse_loss(
                    sampled_returns[indices], predicted_values
                )
